...
// Configuration lives in the existing ServiceConfig key/value store: a platform-level default
// (tenant_id IS NULL) plus optional per-tenant overrides — the SAME table + precedence pattern
// proven by treasury-api's backup/destination.Store.Resolve. Unlike that package this value is a
// plain, non-secret bool, so there is no encryption to do.
