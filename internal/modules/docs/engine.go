// Package docs renders professional, tenant-branded A4 report documents (PDF/CSV) for inventory.
//
// It is the inventory counterpart of the treasury-api docs engine: the same visual language — a
// dynamic brand-color palette derived from the tenant primary color, a logo+title header with a
// gradient rule, a meta strip, summary cards, zebra-striped data tables, key/value blocks and a
// footer — but shaped for TABULAR REPORTS (stock valuation, deadstock, food-cost variance, menu
// engineering) rather than invoice documents.
//
