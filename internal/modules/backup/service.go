// Package backup produces TENANT-SCOPED logical exports of inventory's own data. A backup
...
import (
	...
	entbackup "github.com/bengobox/inventory-service/internal/ent/backup"
)
...
func (s *Service) Generate(ctx context.Context, tenantID uuid.UUID) (Info, error) {
	...
	name := fmt.Sprintf("inventory_backup_%s.json.gz", time.Now().UTC().Format("20060102_150405"))
