...
// for the inventory backup module. The local PVC copy produced by the backup
...
// rest with AES-256-GCM using a Cipher (inventory derives one from SECRET_KEY), so
...
// and portable across services: services with a KeyProvider can pass that;
// services without one (like inventory) can pass any implementation (e.g. a
// SECRET_KEY-derived static key).
...
type Destination struct {
	...
	// under (e.g. "inventory/backups", an S3 key prefix, or a Drive folder path).
...
// single owner of the encrypt/decrypt + upsert logic (encrypt with the PRIMARY
// key; decrypt by trying each candidate key in priority order).
type Store struct {
	client *ent.Client
	keys   Cipher
	logger *zap.Logger
