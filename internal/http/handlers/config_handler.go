...
// ServiceConfigHandler handles service-level configuration CRUD for inventory-api.
...
type inventoryServiceConfigResponse struct {
...
func toInventorySCResponse(cfg *ent.ServiceConfig, isOverride bool) inventoryServiceConfigResponse {
	val := cfg.ConfigValue
	if cfg.IsSecret {
		val = "***"
	}
	return inventoryServiceConfigResponse{
		ID:          cfg.ID,
...
func (h *ServiceConfigHandler) ListPlatformSettings(w http.ResponseWriter, r *http.Request) {
	...
	for _, cfg := range configs {
		r := inventoryServiceConfigResponse{
			ID:          cfg.ID,
			TenantID:    cfg.TenantID,
			ConfigKey:   cfg.ConfigKey,
...
func (h *ServiceConfigHandler) UpsertPlatformSetting(w http.ResponseWriter, r *http.Request) {
	...
	respondJSON(w, http.StatusOK, toInventorySCResponse(cfg, false))
}
...
func (h *ServiceConfigHandler) ListTenantSettings(w http.ResponseWriter, r *http.Request) {
	...
	result := make([]inventoryServiceConfigResponse, 0, len(platformConfigs))
	for _, pc := range platformConfigs {
		if tc, ok := tenantByKey[pc.ConfigKey]; ok {
			result = append(result, toInventorySCResponse(tc, true))
			delete(tenantByKey, pc.ConfigKey)
		} else {
			result = append(result, toInventorySCResponse(pc, false))
		}
	}
