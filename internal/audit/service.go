...
// fraud-relevant actions (stock adjustments, transfers, write-offs, role and
// permission changes, user-outlet assignments, …).
//
// Record is best-effort but durable: it writes synchronously and logs (rather
// than returns) failures so that a failed audit write never breaks the primary
// business operation.
package audit
...
// Record persists an audit entry. Failures are logged, never returned, so the
