...
type BundleQuery struct {
	...
	withItem       *ItemQuery
	withComponents *BundleComponentQuery
...
func (_q *BundleQuery) QueryItem() *ItemQuery {
...
func (_q *BundleQuery) QueryComponents() *BundleComponentQuery {
...
func (_q *BundleQuery) WithItem(opts ...func(*ItemQuery)) *BundleQuery {
	query := (&ItemClient{config: _q.config}).Query()
...
func (_q *BundleQuery) WithComponents(opts ...func(*BundleComponentQuery)) *BundleQuery {
	query := (&BundleComponentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BundleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Bundle, error) {
	...
	if query := _q.withComponents; query != nil {
		if err := _q.loadComponents(ctx, query, nodes,
			func(n *Bundle) { n.Edges.Components = []*BundleComponent{} },
			func(n *Bundle, e *BundleComponent) { n.Edges.Components = append(n.Edges.Components, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *BundleQuery) loadItem(ctx context.Context, query *ItemQuery, nodes []*Bundle, init func(*Bundle), assign func(*Bundle, *Item)) error {
...
func (_q *BundleQuery) loadComponents(ctx context.Context, query *BundleComponentQuery, nodes []*Bundle, init func(*Bundle), assign func(*Bundle, *BundleComponent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(bundlecomponent.FieldBundleID)
	}
	query.Where(predicate.BundleComponent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(bundle.ComponentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "bundle_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
