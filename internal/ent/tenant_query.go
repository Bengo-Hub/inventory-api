...
func (_q *TenantQuery) QueryItems() *ItemQuery {
...
func (_q *TenantQuery) WithItems(opts ...func(*ItemQuery)) *TenantQuery {
	query := (&ItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) loadWarehouses(ctx context.Context, query *WarehouseQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Warehouse)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(warehouse.FieldTenantID)
	}
...
func (_q *TenantQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Item)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(item.FieldTenantID)
	}
