...
func (_q *RecipeIngredientQuery) QueryUnit() *UnitQuery {
...
// QuerySubRecipe chains the current query on the "sub_recipe" edge.
func (_q *RecipeIngredientQuery) QuerySubRecipe() *RecipeQuery {
	query := (&RecipeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *RecipeIngredientQuery) Clone() *RecipeIngredientQuery {
	...
	return &RecipeIngredientQuery{
		...
		withSubRecipe: _q.withSubRecipe.Clone(),
...
func (_q *RecipeIngredientQuery) WithUnit(opts ...func(*UnitQuery)) *RecipeIngredientQuery {
	query := (&UnitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithSubRecipe tells the query-builder to eager-load the nodes that are connected to
// the "sub_recipe" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RecipeIngredientQuery) WithSubRecipe(opts ...func(*RecipeQuery)) *RecipeIngredientQuery {
	query := (&RecipeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSubRecipe = query
	return _q
}
...
func (_q *RecipeIngredientQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RecipeIngredient, error) {
	...
	if query := _q.withUnit; query != nil {
		if err := _q.loadUnit(ctx, query, nodes, nil,
			func(n *RecipeIngredient, e *Unit) { n.Edges.Unit = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubRecipe; query != nil {
		if err := _q.loadSubRecipe(ctx, query, nodes, nil,
			func(n *RecipeIngredient, e *Recipe) { n.Edges.SubRecipe = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RecipeIngredientQuery) loadUnit(ctx context.Context, query *UnitQuery, nodes []*RecipeIngredient, init func(*RecipeIngredient), assign func(*RecipeIngredient, *Unit)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*RecipeIngredient)
	for i := range nodes {
...
func (_q *RecipeIngredientQuery) loadSubRecipe(ctx context.Context, query *RecipeQuery, nodes []*RecipeIngredient, init func(*RecipeIngredient), assign func(*RecipeIngredient, *Recipe)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*RecipeIngredient)
	for i := range nodes {
		if nodes[i].SubRecipeID == nil {
			continue
		}
		fk := *nodes[i].SubRecipeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sub_recipe_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RecipeIngredientQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUnit != nil {
			_spec.Node.AddColumnOnce(recipeingredient.FieldUnitID)
		}
		if _q.withSubRecipe != nil {
			_spec.Node.AddColumnOnce(recipeingredient.FieldSubRecipeID)
		}
	}
