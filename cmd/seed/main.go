...
func main() {
	...
	for _, slug := range tenantsToSeed {
		...
		// Warehouses are synced from auth-api via auth.outlet.* NATS events
		// (inventory branch_subscriber creates warehouse rows when outlet events arrive).
		// Do not seed warehouses here — that would duplicate auth-api data with mismatched UUIDs.
