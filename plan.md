...
- **Seed & RBAC:** inventory-api has **no local Role/Permission schema** and **no cmd/seed** for core data. Tenants and warehouse/outlet data come from **ordering-backend** (tenant sync; inventory-api is in `tenantSyncDestinations`). RBAC is enforced via **auth-api JWT** (shared auth client); auth-api is the source of truth for roles and permissions. For inventory resources, auth-api should define the **eight actions** (add, read, read_own, change, change_own, delete, manage, manage_own) per resource. Resource list and permission codes: see **`docs/rbac-and-seed.md`** (items, warehouses, adjustments, transfers, purchase_orders, reservations).
- **DevOps (aligned with reference apps):** Build script `build.sh`, image `Dockerfile`, and workflow `.github/workflows/deploy.yml` follow the auth-api/notifications-api pattern: sync-secrets job, verify-secrets step, centralized devops-k8s (`devops-k8s/apps/inventory-api/values.yaml`, `apps/inventory-ui/values.yaml`), GIT_COMMIT_ID image tags, and `scripts/infrastructure/create-service-database.sh` / `update_helm_values` for Helm value updates.
...
- **Auth Service:** Issues SSO tokens and tenant/outlet discovery callbacks to hydrate metadata before inventory events are processed. **RBAC:** auth-api is the source of truth for roles and permissions; inventory-api does not maintain its own role/permission seed. JWT validation and scope enforcement are done via shared auth client; frontend (inventory-ui) loads user and RBAC from auth-api `GET /me` with TanStack Query and TTL. See **`docs/rbac-and-seed.md`** for the eight permission actions (add, read, read_own, change, change_own, delete, manage, manage_own) per resource and core data (tenants/warehouses via sync; no local seed). **Redis:** Used for caching, rate limiting, reservation queues (see Technical Foundations). **Events:** NATS JetStream (outbox pattern) for stock adjustments, PO status, replenishment events; health check validates Postgres, Redis, NATS.
